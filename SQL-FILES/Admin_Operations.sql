...
select U.user_id,U.first_name,U.last_name,R.route_name,R.route_cost,B.booking_id,P.payment_date
FROM booking_tickets B join users u on u.user_id=b.user_id
join routes r on R.route_id=B.route_id 
join payment_option P on P.booking_id=B.booking_id;

select U.user_id,U.first_name,U.last_name ,B.booking_id
 from users U join booking_tickets  B on B.user_id=U.user_id 
 where U.user_id=1;
